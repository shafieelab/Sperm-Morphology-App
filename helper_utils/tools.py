...
def testing_sperm_slides(loader, model, logs_path, slide_annotation_file_path, num_classes, print_info=True):
    ...
    for key, images in itertools.groupby(all_values_CSV, lambda x: x[0]):
        ...
        if slide_annotation_file_path is None:
        ...
        if print_info:
            print("|", key + ":-", "| Good Sperm ", round(zero_per, 3),
                  '%  | Good/Total Counts: ' + str(count0) + '/' + str(total), " |")
    ...
    return logs_path + '/slide_prediction_sperm.csv'
...
def validation_loss(loader, model, num_classes, logs_path, data_name='valid_source', dset="", num_iterations=0,
                    is_training=True,
                    ret_cm=False):
    start_test = True
    with torch.no_grad():
    ...
    str(num_iterations) if is_training else "Final") + '_confidence_values_.csv',
        ...
        csv_writer.writerow(
            ['Image_Name', 'Label', 'Prediction'] + list(map(lambda x: 'class_' + str(x) + '_conf', np.arange(
                num_classes))))  # ['Image_Name', 'Prediction', 'class_0_conf', 'class_1_conf']
